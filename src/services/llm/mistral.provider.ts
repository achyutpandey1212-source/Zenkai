...
function estimateCost(model: string, promptTokens: number, completionTokens: number): number {
  const PRICING: Record<string, { input: number; output: number }> = {
    ...
    "open-mixtral-8x22b": { input: 0.65 / 1_000_000, output: 0.65 / 1_000_000 },
    ...
    while (true) {
      try {
        ...
        if (isRetryable && attempt <= maxRetries) {
          ...
          console.warn(
            `[Mistral] Retryable error (Attempt ${attempt}/${maxRetries}). Retrying in ${delay}ms... Reason: ${errorMessage}`
          );

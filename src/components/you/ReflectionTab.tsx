...
import { Compass, Clock, Sparkles, RefreshCw, History, Eye } from "lucide-react";
...
export default function ReflectionTab({ userName }: ReflectionTabProps) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300 w-full text-left">
        ...
        <div className="flex flex-col gap-4 z-10">
          {primaryReflection ? (
            <div className="flex flex-col gap-2">
              ...
              <p className="font-heading text-lg italic text-foreground leading-relaxed max-w-xl">
            ...
            <div>
              ...
              <h2 className="font-heading text-lg font-light text-foreground mt-1">
      ...
      <section className="space-y-4">
        ...
        {totalActive === 0 ? (
          <div className="text-center py-12 bg-secondary/20 rounded-3xl border border-dashed border-border/80 flex flex-col items-center gap-3">
            ...
            <p className="font-sans text-xs text-muted-foreground max-w-sm">No active behavior reflections recorded yet. Zenkai requires repeated evidence over multiple days to map behavioral patterns.</p>
          </div>
        ) : (
            ...
            {activeReflections.map((r, idx) => {
              ...
              return (
                <div 
                  ...
                  className="bg-card border border-border/80 p-6 rounded-2xl flex flex-col gap-4 hover:border-accent/30 transition-all duration-300"
                >
                  <div className="flex justify-between items-start gap-4">
                    <div>
                      ...
                      <h3 className="font-heading text-lg font-medium text-foreground mt-0.5">
                        {r.title}
                      </h3>
                    </div>
                  ...
                  <div>
                    ...
                    {isExpanded && (
                      <div className="mt-4 border-t border-border/30 pt-4 space-y-4 animate-in fade-in duration-200">
      ...
      <footer className="border-t border-border/40 pt-6 flex flex-col gap-4">
        ...
        {devMode && (
          <div className="bg-secondary/40 border border-border p-4 rounded-2xl flex flex-col gap-4 animate-in fade-in duration-200">
            <div className="flex justify-between items-center border-b border-border/60 pb-3">
              ...
              <button
                onClick={handleForceReevaluate}
                disabled={reEvaluating}
                className="font-sans text-[9px] tracking-wider uppercase font-semibold bg-accent text-background px-3 py-1.5 rounded-lg hover:bg-accent/90 transition-colors flex items-center gap-1.5 disabled:opacity-50 cursor-pointer"
              >
                <RefreshCw size={10} className={reEvaluating ? "animate-spin" : ""} />

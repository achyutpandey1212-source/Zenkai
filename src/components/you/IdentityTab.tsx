...
import { Compass, ChevronDown, ChevronUp, History, Eye, Check, X, Clock, RefreshCw } from "lucide-react";
...
export default function IdentityTab() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300 w-full text-left">
      ...
      <section className="bg-card border border-border/75 p-6 rounded-3xl flex flex-col gap-5 relative overflow-hidden">
          ...
          <div className="space-y-3">
            {proposals.map((p, idx) => (
              <div 
                ...
                className="bg-card border border-accent/40 shadow-[0_2px_12px_rgba(201,168,106,0.06)] p-6 rounded-2xl flex flex-col gap-4 relative overflow-hidden transition-all duration-300"
              >
                <div className="flex justify-between items-start">
                  ...
                  <span className="font-sans text-[10px] font-semibold text-accent whitespace-nowrap bg-accent/10 px-2.5 py-0.5 rounded-full border border-accent/20">
                ...
                <div className="flex gap-2.5 mt-1 justify-end">
                  <button 
                    ...
                    className="font-sans text-[10px] tracking-wider uppercase font-semibold bg-accent text-background px-4 py-2 rounded-lg hover:bg-accent/90 transition-colors flex items-center gap-1.5 cursor-pointer"
                  >
                  ...
                  <button 
                    ...
                    disabled={processingProposalId !== null}
                    className="font-sans text-[10px] tracking-wider uppercase font-semibold bg-secondary/80 text-foreground border border-border px-4 py-2 rounded-lg hover:bg-secondary transition-colors flex items-center gap-1.5 cursor-pointer"
                  >
                    <X size={11} /> Dismiss
                  </button>
                  <button 
                    ...
                    disabled={processingProposalId !== null}
                    className="font-sans text-[10px] tracking-wider uppercase font-semibold bg-transparent text-muted-foreground border border-border px-3 py-2 rounded-lg hover:bg-secondary/40 transition-colors cursor-pointer"
                  >
                    Later
                  </button>
                </div>
      ...
      <section className="space-y-4">
        <div className="flex justify-between items-center">
          <span className="font-sans text-[9px] tracking-[0.25em] text-accent font-bold uppercase">
            Core Blueprint Traits
          </span>
          <button
            onClick={handleForceReevaluate}
            disabled={reEvaluating}
            className="font-sans text-[9px] tracking-wider uppercase font-semibold text-muted-foreground hover:text-foreground cursor-pointer flex items-center gap-1 transition-colors disabled:opacity-50"
          >
        ...
        {allActive.length === 0 ? (
          <div className="text-center py-12 bg-secondary/20 rounded-3xl border border-dashed border-border/80 flex flex-col items-center gap-3">
            <Compass size={24} className="text-accent/40" />
            <p className="font-sans text-xs text-muted-foreground max-w-sm">No confirmed identity traits yet. Zenkai will automatically identify traits from memory as you converse.</p>
          </div>
        ) : (
            ...
            {allActive.map((t, idx) => {
              ...
              return (
                <div 
                  ...
                  className="bg-card border border-border/80 p-5 rounded-2xl flex flex-col gap-3 hover:border-accent/30 transition-all duration-300"
                >
                  ...
                  {t.evidence && (
                    <div className="mt-1">
                      <button 
                        ...
                        className="font-sans text-[9px] tracking-wider uppercase text-accent font-semibold flex items-center gap-1 hover:text-accent/80 transition-colors cursor-pointer"
                      >
                        {expandedEvidence[t._id] ? "Hide Evidence" : "View Evidence"}
                      </button>
                      
                      {expandedEvidence[t._id] && (
                        <div className="mt-2 bg-secondary/30 border border-border/40 p-3 rounded-lg text-[11px] font-sans text-muted-foreground leading-relaxed whitespace-pre-line animate-in fade-in duration-200">
                          {t.evidence}
                        </div>
                      )}
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {profile.emergingTraits.map((t, idx) => (
              <div 
                ...
                className="bg-card border border-border/75 p-5 rounded-2xl flex flex-col gap-2 hover:border-accent/20 transition-all duration-300"
              >
                <div className="flex justify-between items-center">
      ...
      <footer className="border-t border-border/40 pt-6 flex flex-col gap-4">
        <div className="flex justify-between items-center">
          <button 
            ...
            className="font-sans text-[9px] tracking-wider uppercase text-muted-foreground font-semibold hover:text-accent transition-colors flex items-center gap-1 cursor-pointer"
          >
